...
async function guardar() {
   ...
   error.value = null
   try {
      await $fetch('/api/auth/password', { method: 'PATCH', body: { ...form } })
      form.passwordActual = ''
      form.password = ''
...
<template>
   <div class="space-y-6">
      <p class="text-sm text-usm-text-muted dark:text-slate-400">
         Cambia la contraseña con la que ingresas al sistema.
      </p>

      <div class="max-w-lg rounded-2xl border border-default bg-default p-4 sm:p-6">
         <UForm id="form-contrasena" :state="form" class="space-y-4" @submit="guardar">
            ...
            <div class="flex justify-end">
               <UButton type="submit" form="form-contrasena" :loading="guardando" :disabled="!puedeGuardar">
                  Cambiar contraseña
               </UButton>
            </div>
         </UForm>
