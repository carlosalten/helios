...
// propósito: este reporte procesa los archivos enteramente en el browser, sin sesión ni fetch.
...
async function calcular() {
   ...
   try {
      ...
      if (estudiantes.size === 0) {
         error.value =
            'No se encontraron estudiantes en las listas. Verifica que los archivos sean listas SIGA válidas.'
         return
      }

      // Cada celda: Map<rut, EstudianteDetalle> para contar únicos y guardar detalle
      const detalleMap: Map<string, EstudianteDetalle>[][] = Array.from({ length: CANT_BLOQUES }, () =>
...
function estadoCelda(bloqueIdx: number, diaIdx: number): EstadoCelda {
   ...
   const esColacion =
      !colacionDisponible.value && (bloqueIdx === 6 || bloqueIdx === 7) && (diaIdx === 1 || diaIdx === 3)
   ...
   <div>
      <div class="p-6 overflow-hidden rounded-2xl border border-default bg-default">
         <div class="flex items-center gap-4 mb-8">
            <div
               class="w-12 h-12 rounded-xl bg-usm-green-50 dark:bg-usm-green-950 flex items-center justify-center shrink-0"
            >
               <UIcon
                  name="i-heroicons-calendar-days-16-solid"
                  class="w-6 h-6 text-usm-green-700 dark:text-usm-green-400"
               />
            </div>
            <div>
               <p class="text-usm-text-muted dark:text-slate-400 text-sm">
                  Disponibilidad horaria común de un grupo de estudiantes
               </p>
            </div>
         </div>
         ...
         <div class="lg:grid lg:grid-cols-[380px_1fr] lg:gap-8 lg:items-start">
               ...
               <div class="grid gap-4 mb-6">
                  ...
                  <UCard class="border border-default">
                     ...
                     <p class="text-usm-text-muted dark:text-slate-400 text-xs mt-0.5">
                        Archivo <span class="font-mono">Direst_050_TU.csv</span> con carga horaria de todos los
                        estudiantes
                     </p>
                     <div class="flex items-center justify-between gap-2 mt-3">
                        <span
                           v-if="direstArchivo"
                           class="text-xs text-usm-text-muted dark:text-slate-400 truncate font-mono"
                        >
                           {{ direstArchivo.name }}
                        </span>
                        <span v-else />
                  ...
                  <UCard class="border border-default">
                     ...
                     <p class="text-usm-text-muted dark:text-slate-400 text-xs mt-0.5">
                        Una o más listas de estudiantes exportadas desde SIGA
                     </p>
                     ...
                     <div v-if="listasArchivos.length > 0" class="flex flex-col gap-1.5">
                        <div
                           v-for="(archivo, idx) in listasArchivos"
                           :key="archivo.name"
                        ...
                        >
                           <div class="flex items-center gap-2 min-w-0">
                              ...
                              <span class="text-sm text-usm-text dark:text-slate-200 truncate font-mono">{{
                                 archivo.name
                              }}</span>
                           </div>
                           <UButton
                              ...
                              class="shrink-0 ml-2"
                              @click="quitarLista(idx)"
                           />
                        </div>
                     </div>
                     <p v-else class="text-sm text-usm-text-muted dark:text-slate-400 text-center py-1">
                        Sin listas agregadas
                     </p>
                  </UCard>

                  <!-- Colacion toggle -->
                  <UCard class="border border-default">
                     <p class="font-semibold text-usm-text dark:text-slate-100 text-sm">
                        Bloques protegidos (Mar/Jue, Bloques 7–8)
                     </p>
                     <p class="text-usm-text-muted dark:text-slate-400 text-xs mt-0.5">
                     ...
                     <div class="flex items-center gap-3 mt-3">
                        <span
                           ...
                           :class="
                              colacionDisponible
                                 ? 'text-usm-yellow-700 dark:text-usm-yellow-400'
            ...
            <div v-if="resultado" class="mt-8 lg:mt-0">
               <div class="flex flex-wrap items-center gap-2 mb-5">
                  ...
                  <UBadge
                     v-for="curso in resultado.cursos"
                     :key="curso"
                     color="primary"
                     variant="solid"
                     class="text-white"
               ...
               <div class="flex flex-wrap gap-4 mb-4 text-xs text-usm-text-muted dark:text-slate-400">
                  <div class="flex items-center gap-1.5">
                     <div
                        class="w-3.5 h-3.5 rounded bg-usm-green-200 dark:bg-usm-green-900 border border-usm-green-400 dark:border-usm-green-700"
                     ></div>
                     Libre
                  </div>
                  <div class="flex items-center gap-1.5">
                     <div
                        class="w-3.5 h-3.5 rounded bg-usm-yellow-100 dark:bg-usm-yellow-950 border border-usm-yellow-300 dark:border-usm-yellow-800"
                     ></div>
                     Protegidos
                  </div>
                  <div class="flex items-center gap-1.5">
                     <div
                        class="w-3.5 h-3.5 rounded bg-usm-gray-200 dark:bg-slate-600 border border-usm-gray-300 dark:border-slate-500"
                     ></div>
                     Ocupado
                  </div>
               </div>
                  ...
                  <table class="w-full text-sm border-collapse">
                     ...
                     <tbody>
                        <template v-for="b in CANT_BLOQUES" :key="b">
                           <tr
                              v-if="b === 9"
                              class="border-t-2 border-usm-blue-200 dark:border-usm-blue-800 bg-usm-blue-50 dark:bg-usm-blue-950"
                           >
                              <td
                                 :colspan="1 + resultado.dias.length"
                           ...
                           <tr
                              ...
                              :class="
                                 b % 2 === 0 ? 'bg-usm-gray-50 dark:bg-slate-800/50' : 'bg-white dark:bg-slate-900'
                              "
                           >
                              <td class="px-4 py-2.5">
                                 <div class="font-semibold text-usm-text dark:text-slate-200 text-xs">
                                    Bloque {{ b }}
                                 </div>
                                 <div class="text-usm-text-muted dark:text-slate-400 text-xs font-mono mt-0.5">
                              ...
                              <td v-for="(_, d) in resultado.dias" :key="d" class="px-2 py-2 text-center">
                                 <UTooltip :text="rutsCelda(b - 1, d)" :disabled="estadoCelda(b - 1, d) !== 'ocupado'">
                                    <div
                                       ...
                                       :class="{
                                          'bg-usm-green-100 dark:bg-usm-green-950 border border-usm-green-300 dark:border-usm-green-800 text-usm-green-700 dark:text-usm-green-400':
                                             estadoCelda(b - 1, d) === 'libre',
                                          'bg-usm-yellow-50 dark:bg-usm-yellow-950 border border-usm-yellow-300 dark:border-usm-yellow-800 text-usm-yellow-700 dark:text-usm-yellow-400':
                                             estadoCelda(b - 1, d) === 'colacion',
                                          'bg-usm-gray-100 dark:bg-slate-700 border border-usm-gray-200 dark:border-slate-600 text-usm-gray-600 dark:text-slate-300 cursor-pointer hover:bg-usm-gray-200 dark:hover:bg-slate-600 transition-colors':
                                             estadoCelda(b - 1, d) === 'ocupado',
                                       }"
                                       @click="estadoCelda(b - 1, d) === 'ocupado' && abrirModal(b - 1, d)"
                                    >
                                       <UIcon
                                          v-if="estadoCelda(b - 1, d) === 'libre'"
                                          name="i-heroicons-check-16-solid"
                                          class="w-4 h-4"
                                       />
                                       <UIcon
                                          v-else-if="estadoCelda(b - 1, d) === 'colacion'"
         ...
         <UModal
            ...
            :ui="{
               ...
               title: 'text-usm-blue dark:text-usm-cyan font-semibold',
            ...
            <template #body>
               ...
               <div v-else class="overflow-x-auto">
                  <table class="w-full text-xs border-collapse">
                     ...
                     <tbody>
                        <tr
                           v-for="est in estudiantesModal"
                           :key="est.rut"
                           class="border-b border-usm-border dark:border-slate-700 last:border-0 hover:bg-usm-gray-50 dark:hover:bg-slate-700"
                        >
                           <td
                              class="py-2.5 px-3 font-mono text-xs text-usm-text dark:text-slate-200 whitespace-nowrap"
                           >
                              {{ est.rut }}
                           </td>
